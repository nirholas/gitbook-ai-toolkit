 ...
 * 
 * Usage:
 *   tsx src/example-extractor.ts ./output/sentry
 *   tsx src/example-extractor.ts ./output/stripe --language typescript
 */
...
class CodeExampleExtractor {
  ...
  /**
   * Extract all code examples from scraped GitBook documentation
   */
  async extract(): Promise<void> {
    console.log('📚 Extracting code examples from GitBook documentation...');
  ...
  private extractFromMarkdown(markdown: string, source: string): CodeExample[] {
    ...
    while ((match = codeBlockRegex.exec(markdown)) !== null) {
      ...
      // Check for filename in GitBook format
      const filenameMatch = beforeCode.match(/###\s+([^\n]+\.[\w]+)\n*$/);
...
async function main() {
  ...
  if (args.length === 0 || args.includes('--help')) {
    console.log(`
Code Example Extractor for GitBook
===================================

Extract and organize code examples from scraped GitBook documentation
...
Examples:
  tsx src/example-extractor.ts ./output/sentry
  tsx src/example-extractor.ts ./output/stripe --language typescript
  tsx src/example-extractor.ts ./output/gitcoin --output ./examples
