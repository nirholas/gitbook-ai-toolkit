 ...
 * 
 * Usage:
 *   tsx src/mcp-generator.ts ./output/sentry
 *   tsx src/mcp-generator.ts ./output/stripe --output ./mcp-tools
 */
...
class McpToolGenerator {
  ...
  /**
   * Generate MCP tools from scraped GitBook documentation
   */
  async generate(): Promise<void> {
    console.log('🔧 Generating MCP tools from GitBook documentation...');
    ...
    console.log(`\n✅ Generated ${tools.length} MCP tools from GitBook`);
  ...
  private parseApiToMcpTool(markdown: string, page: any): McpTool | null {
    try {
      ...
      const methodMatch = markdown.match(/\*\*Method:\*\* `(GET|POST|PUT|DELETE|PATCH|HEAD|OPTIONS)`/);
      ...
      // Build input schema with examples
      ...
      for (const param of parameters) {
        properties[param.name] = {
          ...
          example: this.generateExample(param.type, param.name)
        };
      ...
      const examples = this.extractCodeExamples(markdown, parameters);
  ...
  private async generateTypeScriptFile(tools: McpTool[]): Promise<void> {
    let typescript = `/**
 * Auto-generated MCP Tools from GitBook Documentation
...
`;
   ...
   */
  private generateExample(type: string, name: string): any {
    ...
    if (lowerName.includes('token')) return 'sk_test_xxxx';
    if (lowerName.includes('key')) return 'api_key_xxxx';
  ...
  ): Array<{ description: string; input: Record<string, any>; output?: string }> {
    const examples: Array<{ description: string; input: Record<string, any>; output?: string }> = [];
    
    // Look for code blocks
    const codeBlocks = markdown.matchAll(/```(?:json|javascript|typescript|curl)?\n([\s\S]*?)```/g);
    
    for (const match of codeBlocks) {
      const code = match[1].trim();
      
      // Try to parse as JSON (request example)
      try {
        if (code.startsWith('{') || code.startsWith('[')) {
          const parsed = JSON.parse(code);
          
          // Check if it matches our parameters
          const matchingParams = parameters.filter(p => p.name in parsed);
...
async function main() {
  ...
  if (args.length === 0 || args.includes('--help')) {
    console.log(`
MCP Tool Generator for GitBook
===============================

Automatically generate MCP tool definitions from scraped GitBook documentation
...
Examples:
  tsx src/mcp-generator.ts ./output/sentry
  tsx src/mcp-generator.ts ./output/stripe --output ./custom-path
